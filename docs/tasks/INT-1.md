...
- **Output contract:** A full investigation triggered from the UI completes all 6 workflow steps:
  ...
  3. Deep investigate suspicious listings
  4. Cluster sellers
...
- **Acceptance criteria:**
  ...
  - At least 1 finding visible in Evidence panel
  - At least 1 route visible on map (or in route query)
  - Case file appears in Case panel with executive summary
...
- **Gotchas:**
  ...
  - Test with a SIMPLE prompt first: one region, one marketplace (e.g., just Amazon.de)
